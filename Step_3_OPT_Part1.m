...
%===============================================================================================
    ...
    % Entropy Encoding
    fileIDlow = sprintf('encodeddata_framelow%d.txt', framenumber);
    [codedictionarylow,bitcountlow] = getentropycoding(quantizedblocksl, fileIDlow);
    fprintf('Bitcount for low quality for frame%d : %s\n', framenumber, num2str(bitcountlow));
    
    % for medium quality
    quality ='medium';
    quantizedblocksm = getquantized(DCTblocks, quality);
    % Entropy Encoding
    fileIDmed = sprintf('encodeddata_framemed%d.txt', framenumber);
    [codedictionarymed,bitcountmed] = getentropycoding(quantizedblocksm, fileIDmed);
    fprintf('Bitcount for medium quality for frame%d : %s\n', framenumber, num2str(bitcountmed));
    
    %for high quality
    quality ='high';
    quantizedblocksh = getquantized(DCTblocks, quality);
    % Entropy Encoding
    fileIDhigh = sprintf('encodeddata_framehigh%d.txt', framenumber);
    [codedictionaryhigh,bitcounthigh] = getentropycoding(quantizedblocksh, fileIDhigh);
    fprintf('Bitcount for high quality for frame%d : %s\n', framenumber, num2str(bitcounthigh));
    ...
    % For a given bitrate
    bitrate = 1500*1000 ; % 1.5Mbps  <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    %Target compression ratio:
    tgtcr = bitlength/bitrate;
    % display the target compression ratio:
    fprintf('Target compression ratio for frame%d : %s\n', framenumber, num2str(tgtcr));
    % Lets take tgtcr as a input argument to modified qunatization function
    fprintf('\n');  % This will print a blank line
...
%=======================================================================================================================
    ...
    fprintf('\n');  % This will print a blank line
    fprintf('Achieved CR for frame%d : %s\n', framenumber, num2str(crnew));
    fprintf('Achieved QP for frame%d : %s\n', framenumber, num2str(QP));
    fprintf('\n\n');  % This will print a blank line
...
%======================================================================================================================
    ...
    % TRANSMISSION HAPPENS HERE !!! (MVs, FileIDs, Code dictionaries and QPs are TRANSMITTED SEPERATELY)
    ...
    invquantblocks = getinversequantizedmodified(decodedblocks,QP);
...
psnrvlI = psnr(reIframe,grayimageori);
fprintf('Achieved PSNR for frame1 : %s\n', num2str(psnrvlI));
fprintf('\n');  % This will print a blank line

%==========================================================================

% RECONSTRUCT THE CURRENT FRAMES USING MVS AND RESIDUALS
...
function motionvector = getmotionvector(previousmbs, currentmbs)
    ...
    for i = 1:cellh
        for j = 1:cellw
            ...
            % Go with a Sequential Adaptive search (Not Logrithmic)
